...
class SubcategoryPage(utils.DataMixin, generic.ListView):
    ...
    sort = {'popular': 'по популярности', 'price': 'по возрастанию цены',
    ...
    def get_queryset(self):
        ...
        if self.values:
            return self.subcategory.products.filter(
            ...
            return (
                self.subcategory.products.filter(Q(price__gte=self.min_price) &
                                                 Q(price__lte=self.max_price) &
                                                 Q(available=True)).order_by(
                    self.sorted_field))

    def get_properties(self, queryset):
        return {p_type: [p_val for p_val in
                         models.PropertyValue.objects.filter(
                             Q(properties__type=p_type) & Q(
                                 properties__product__in=self.subcategory.products.all()))]
                for p_type in
